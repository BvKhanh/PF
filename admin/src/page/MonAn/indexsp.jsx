...
export default function Indexsp() {
    ...
    const deleteSP = (ma_mon_an) => {
        if(window.confirm("Bạn có muốn xóa sản phẩm này không ?")){
            axios.delete(`http://localhost:5000/api/deletesp/${ma_mon_an}`)
                ...
                <table className="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        {data.map((item,index)=>{
                            ...
                            return(  
                            <tr key={item.ma_mon_an}>
                                <td>{stt}</td>
                                <td><img style={{borderRadius: '5px'}} src={item.anh_monan} width='60' height='60' className="img img-responsive" /></td>
                                <td>{item.ten_mon_an}</td>
                                <td>{item.soluong}</td>
                                <td><Link to={`/Viewsp/${item.ma_mon_an}`} type="button" className="btn btn-primary">Chi Tiết</Link></td>
                                <td><Link to={`/Updatesp/${item.ma_mon_an}`} className="btn btn-warning">Sửa</Link></td>
                                <td>
                                        <button type='submit' onClick={()=> deleteSP(item.ma_mon_an)} className='btn btn-danger'>Xóa</button>
                                </td>
                            </tr>)
